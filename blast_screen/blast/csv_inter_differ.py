...
def scratch(l):
    ...
    for e in l:
        ...
        if e['igemid'] not in igem_id and flag != 0:
            ...
            if inter_compare(l_e):
                l_c += l_e
            l_e = []
        flag = 1
        li = ['All', 'Reporter', 'Coding', 'Composite', 'Device', 'Generator', 'Inverter', 'Measurement', 'Plasmid',
              'Protein_Domain', 'Signalling', 'Translational_Unit']
        if e['type'] in li:
...
def inter_compare(l):
    ...
    for i in range(len(l)):
        if l[i]['type'] == 'All': flag_all += 1
    # when the current sequence doesn't have type "All"
    if flag_all == 0: return 0
    else:
        flag_feature = len(l) - flag_all
        # if flag_feature differ from flag_all, means the length of these two are differ
        if flag_feature != flag_all: return 1
        else:
            ll = l[flag_all: len(l)]
            ll_mark = []
            # record the special elements of each row
            for i in range(len(l)):
                dic = {}
                dic['unid'] = l[i]['unid']
                dic['start'] = int(l[i]['start_gene']) + int(l[i]['query_start']) - 1
                dic['len'] = l[i]['length']
                ll_mark.append(dic)
            for i in range(flag_all):
                # compare
                if ll_mark[i] not in ll_mark[flag_all: len(l)]: return 1
    return 0
...
list1, keys = readcsv('datasets\/data_after_clean.csv')
list_after_scratch = scratch(list1)

i = 1
with open('datasets\/data_after_clean_differ.csv','w', newline='') as f:
    writer = csv.writer(f)
    first_row = []
